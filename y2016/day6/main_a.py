class PasswordBuilder:
    ...
    def __init__(self, raw_data):
        ...
        for column in column_data:
            self.password += PasswordBuilder.__get_highest_occurance_char_in_list(column)
    ...
    @staticmethod
    def __get_highest_occurance_char_in_list(list_data) -> str:
        letters = dict()
        for c in list_data:
            if c not in letters.keys():
                letters[c] = 0
            letters[c] += 1
        max_used = max(letters.values())

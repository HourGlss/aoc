...
def create_image(_h, _w) -> Image:
    return Image.new("RGBA", (_w, _h), (0, 0, 0, 255))
...
def interact(_image: Image, start: tuple[int, int] = (0, 0), end: tuple[int, int] = (0, 0), action: str = ""):
    pixels = _image.load()
    for y in range(_image.size[0]):  # for every pixel:
        for x in range(_image.size[1]):
            if start[1] <= y <= end[1]:
                if start[0] <= x <= end[0]:
                    if action == "on":
...
def get_actions_from_raw_input(_input: str) -> list[tuple[str, tuple[int, int], tuple[int, int]]] | None:

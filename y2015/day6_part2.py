...
raw_input = """turn on 887,9 through 959,629
...
def create_grid(_h: int, _w: int) -> list[list[int]]:
    _grid = []
    for y in range(_h):
        _grid.append([])
        for x in range(_w):
...
def print_grid(_grid: list[list[int]]):
    h = len(_grid)
    w = len(_grid[0])
    for y in range(h):
        for x in range(w):
...
def interact(_grid: list[list[int]], _start: tuple[int, int], _end: tuple[int, int], _action: str):
    for y in range(_start[1], _end[1] + 1):
        for x in range(_start[0], _end[0] + 1):
            if _action == "on":
                _grid[y][x] += 1
            if _action == "off":
                _grid[y][x] -= 1
...
def count_lit(_grid: list[list[int]]) -> int:
    h = len(_grid)
    w = len(_grid[0])
    lit = 0
    for y in range(h):
...
def get_actions_from_raw_input(_input: str) -> list[tuple[str, tuple[int, int], tuple[int, int]]]:
    lines = _input.replace("turn", "").split("\n")
    _actions = []
    for line in [_.strip() for _ in lines]:
        print(line)
        pieces = line.split(" ")
        current_action, _start, _, _end = pieces
        start_pieces = _start.split(",")
        end_pieces = _end.split(",")
        _actions.append(
            (current_action,
             (int(start_pieces[0]), int(start_pieces[1])),
             (int(end_pieces[0]), int(end_pieces[1])),
             )
        )
    return _actions
...
if __name__ == "__main__":
    ...
    actions = get_actions_from_raw_input(raw_input)
    for action in actions:
        curr_action, start, end = action
        # print(curr_action, start, end)
        interact(grid, start, end, curr_action)
    lit = count_lit(grid)
    print(lit)
